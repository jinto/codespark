...
- `ghostty_surface_set_size` expects **physical pixels** (use `convertToBacking`), and **크기가 그대로면 아무것도 안 한다** — `updateSize`가 첫 줄에서 돌아가고, 이유로 SwiftUI를 명시한다(`embedded.zig`). 다시 보이게 된 탭을 재렌더시키려고 같은 크기로 부르던 코드가 여기 있었다: 렌더는 셸의 출력이나 크기 변화로만 걸리는데 idle 프롬프트에 두고 온 탭엔 둘 다 없어서, 낡은 프레임(빈 화면 + 커서 자국)을 그대로 입고 돌아왔다. 크기 델타 없이 그리게 하는 API는 따로 있다 — **`ghostty_surface_refresh`**(= `queueRender`). 오라클: 출력이 끝난 탭에서 다른 탭에 갔다 오면 화면이 비고 Ctrl-L로 살아나면 그 병이다.
  - **0×0은 넘기지 않는다**(`GhosttyTerminalSurfaceView.reportsSize`). 메인 영역이 `.terminals`를 떠났다 돌아오면(탭 없는 워크트리·프로젝트를 거쳐 오면) SwiftUI가 떼어낸 **모든** 표면의 프레임을 0×0으로 만든다(실측). 그대로 넘기면 Ghostty가 그리드를 1×1로 잡고(`GridSize.update`의 `@max(1, …)`) pty에 SIGWINCH, 몇 ms 뒤 원래 크기로 또 SIGWINCH — Claude Code 같은 전체화면 TUI가 한 줄짜리 화면에 맞춰 그렸다 돌아와 **출력과 입력창 사이에 빈 칸**이 남는다("Jump to bottom"이 같이 뜬다). 공식 Ghostty는 크기를 `GeometryReader`에서 받아 이걸 안 본다; 우리는 `setFrameSize`를 그대로 넘겨서 봤다. 오라클: 탭에서 `trap 'echo WINCH $(stty size) >> f' WINCH; while sleep 0.1; do :; done`을 돌리고 `Cmd` 숫자로 탭 없는 자리를 거쳐 온다 — 크기가 안 변했는데 WINCH가 찍히면 그 병이다.
...
- **추월당한 네비게이션은 손을 뗀다**(`navigation` 세대 번호). 취소는 `detailTask` **안**만 덮는다 — 거기선 모든 분기가 `Task.isCancelled`를 묻는다. 못 덮는 것은 **호출자가 자기 `await` 뒤에 남겨둔 문장**이고, `selectWorktree`에 그게 하나 있었다: `Cmd` 숫자를 빠르게 세 번 누르면 추월당한 두 번째가 왕복에서 돌아와 **방금 도착한 프로젝트에 남의 워크트리 경로를 대입했다**. 그 경로에 맞는 워크스페이스가 없으니 `visibleSessions`가 비고 `didSet`이 활성 탭을 놓아, **탭이 있고 배지가 1이라고 말하는 행 옆에서 메인 영역이 "New Terminal"을 내밀었다**. 다른 프로젝트에 갔다 오면 `apply(detail:)`이 경로를 다시 써서 낫는다 — 그래서 "가끔"으로 보인다.
  - `selectProject`/`selectWorktree`가 **자기가 아직 현재인지 Bool로 돌려준다**. 대입·스크롤·`removeWorktree` 같은 왕복 **뒤의** 행동은 전부 그 뒤에 선다. `startAndAttachSession` 계열이 이미 쓰던 `guard selection.id == project.id`의 일반형이고, 같은 프로젝트 안에서 워크트리만 갈아타는 추월까지 잡으려면 ID 비교로는 부족해서 세대 번호다.
  - **스텁 지연은 취소를 견뎌야 한다**: `MockProjectCoreClient`가 `try? await Task.sleep`으로 기다리면 취소되는 순간 **즉시** 돌아온다. 그러면 낡은 대입이 새 프로젝트의 `apply(detail:)`보다 **먼저** 떨어지고 `apply`가 덮어써서, 고치지 않은 코드도 초록불이 된다. 진짜 왕복은 아무도 안 기다린다고 일찍 오지 않으므로 `Task.detached`로 감싼다(`test_an_overtaken_digit_does_not_empty_the_project_it_was_overtaken_by`).
...
- **전환 수단**: 사이드바 행 클릭 + `Cmd+Opt+[`/`]` 순환 + `Cmd+1…9`. 사이드바를 숨기면 클릭 경로가 사라지므로 핫키가 없으면 다른 워크트리의 탭이 고립된다.
  - **순환도 숫자의 매너를 따른다**(`cycleWorktree`): 트리를 열고(접지는 않는다) 내려선 행으로 스크롤한다. 예전엔 접힌 트리에서 순환하면 타이틀 부제만 바뀌어 **고장으로 읽혔다**. 순환은 빈 워크트리도 밟는다는 점이 숫자와 다르다 — `workspaces` 전체를 걷지, 탭 있는 곳만 골라 뛰지 않는다.
...
프로젝트는 사용자가 만든 **그룹**에 묶을 수 있다(`ProjectGroups`, UserDefaults `projectGroups` 키 하나에 JSON). 어느 그룹에도 없는 프로젝트는 **기본 그룹**이고, 기본 그룹은 저장되지 않으며(= 나머지 전부) **헤더를 그리지 않는다** — 그룹을 하나도 안 만든 사이드바는 예전과 똑같다. 명명 그룹이 위, 기본 그룹이 맨 아래.

- **그룹 안의 순서는 `projectOrder` 그대로다.** 그룹은 소속만 말한다. 그래서 그룹에 넣으면(헤더에 드랍·메뉴·새 그룹) 전역 순서의 **끝**으로 옮겨 그 그룹의 마지막이 된다.
- **드랍 위치가 곧 그룹이다**: 행 위 = 그 행의 그룹, 헤더 = 그 그룹 끝, 목록 아래 빈 공간 = 기본 그룹(맨 아래에 그려지므로).
- **그룹을 지우면 라벨만 지운다** — 프로젝트는 기본 그룹으로 돌아간다. 사라진 그룹을 가리키는 소속은 기본 그룹으로 읽는다(`groupID(of:)`).
- **접힌 그룹은 `Cmd` 숫자를 받지 않는다**(사용자 선택). 워크트리 접힘은 번호를 안 바꾸는 것과 반대다 — 그룹은 화면 밖이고, 9개 숫자를 보이는 곳에 쓰는 게 낫다고 골랐다.
- **헤더와 프로젝트 행은 한 `ForEach`의 형제다**(`SidebarItem`). 헤더 밑에 중첩하면 프로젝트 행이 최상위 id가 아니게 되어 `scrollTo`가 조용히 못 찾는다 — 위 "스크롤은 2단계다"와 같은 함정.
- **`AppModel`이 `defaults`를 주입받는다.** 테스트 호스트는 진짜 앱과 도메인을 공유해서, 예전 재정렬 테스트가 개발자의 `projectOrder`를 매번 지웠다. 그룹엔 사용자가 적은 이름이 들어가므로 테스트는 `UserDefaults(suiteName:)`만 쓴다.
- **검증 주의**: osascript의 AX `click`은 헤더의 `onTapGesture`를 부르지 않는다(실측 — 상태 불변). 진짜 마우스 이벤트(`CGEvent`)로 눌러야 접힘이 토글된다.
...
- **왜 릴리즈까지 갔나 — 그리고 개발자가 못 본 진짜 이유**: `resourcesdir.zig`는 release에서 `GHOSTTY_RESOURCES_DIR`을 검사 없이 채택하고 Debug는 sentinel 탐색 실패 **후** 채택한다(`:102-111`) — **Debug도 결국 채택한다**. 그러니 "Debug는 재현 안 됨"은 틀렸다(실측: Debug 번들의 `TERMINFO`도 죽은 경로를 가리켰다). 진짜 경계는 **"발견 가능한 xterm-ghostty terminfo DB가 있는 기계인가"**다 — Ghostty.app을 써본 기계는 `~/.terminfo/78/xterm-ghostty`가 있어 ncurses가 fallback으로 가려준다. 신고자는 Ghostty가 없어 재현됐다. **검증할 땐 반드시 `HOME` 격리 + `TERMINFO_DIRS` 언셋**으로 그 fallback을 치워야 한다 — 안 하면 `infocmp OK`에 속는다(이 기계에서 실제로 그랬다).
- **sentinel 철자는 우리가 고른 게 아니다**: macOS는 `terminfo/78/xterm-ghostty`(`78`=`x`의 16진), 리눅스는 `x/xterm-ghostty`. `resourcesdir.zig:63`이 하드코딩한다. 게이트가 이 경로를 쓴다.
- **빌드 페이즈는 fail-closed**: terminfo(`78/xterm-ghostty`)나 shell-integration이 없으면 **`exit 1`**(예전엔 경고+`exit 0`). stale `zig-out`이면 둘 다 없는 번들이 조용히 나왔다. 실패 메시지가 `scripts/build-ghostty.sh`를 가리킨다. `--delete`는 각 대상(`Resources/terminfo`, `Resources/ghostty`)으로 스코프된다 — 다른 리소스를 지우지 않게.
- **`setenv`는 그대로 둔다 — terminfo 존재로 게이팅하지 말 것**: `GHOSTTY_RESOURCES_DIR`이 nil이 되면 libghostty가 **shell integration(OSC 7)을 끄고**(`Exec.zig:784-787`) XDG_DATA_DIRS·MANPATH도 잃는다. terminfo만 번들에 넣으면 원래 setenv 경로가 그대로 옳다(TERM 해석 + integration 유지). "안전망"이라며 게이팅하면 방금 고친 cwd 추적을 죽인다(Codex 리뷰가 잡았다).
- **게이트**: `AppBundleMetadataTests`가 **빌드된 앱 번들**(`Bundle.main`, `TEST_HOST`)에서 `terminfo/78/xterm-ghostty` 존재+readable, `ghostty/shell-integration` 존재, terminfo가 ghostty의 형제인지 검사한다. 파일 존재만으론 부족해서 — 화면 렌더는 실셸이 `TERM`을 해석해야 드러나므로 — 검증은 `env -u TERMINFO_DIRS HOME=/빈곳 infocmp -A "$TERMINFO" "$TERM"`로 `cub1`까지 확인한다. 릴리즈는 `codesign --verify --deep --strict`로 terminfo 트리까지 서명 검증한다(심링크가 번들 밖을 가리키면 공증 전에 잡는다 — 현재 DB는 심링크 없음).
- **`project.yml`과 pbxproj를 같이 고친다**: pbxproj가 실제로 도는 것이지만 xcodegen(`build-macos.sh`)이 `project.yml`에서 재생성하므로, 한쪽만 고치면 되돌아간다(`ReleaseWorkflowTests`가 둘의 벌어짐을 감시한다).
  ...
  - **원격에 terminfo도 심는다 — 접속 시, 없을 때만**(`RemoteCwdReporter.terminfoInstaller`). ssh가 pty로 우리 `TERM=xterm-ghostty`를 전파하는데, Ghostty를 안 써본 원격 박스엔 그 terminfo가 없어 백스페이스·라인 편집이 저쪽에서 깨진다 — 로컬 번들 fix(위 "terminfo도 번들에")와 **별개**다(번들은 로컬 셸용, 원격은 원격 DB를 쓴다). nwork001·coin-dataset 둘 다 재현·해결 실증(2026-09-02).
    - **컴파일된 DB는 못 보낸다 — source를 보낸다**: 번들의 컴파일된 엔트리를 `infocmp -A <번들>/terminfo -x`로 source로 되돌려(`RemoteTerminfo.bundledSourceBase64`, 앱 버전당 한 번 캐시) **base64**로 실어 보내고(따옴표·`printf`·ssh를 무사통과), 원격 `tic`이 컴파일한다. `sshCommand(installingTerminfo:)`가 `cd` 뒤·exec/launcher 앞에 preamble로 끼운다.
    - **가드 둘 다 필수**: ① **멱등·비파괴** — `infocmp xterm-ghostty`가 이미 성공하면 아무것도 안 한다. Ghostty 유저의 박스엔 자기 것(더 새것일 수 있음)이 있고 `~/.terminfo`가 시스템 DB를 가리므로, 덮어쓰면 fix가 아니라 회귀다. **특히 원격이 mac이면** 이 가드가 원본을 지킨다. ② **옛 동작으로 degrade** — `tic`이 없거나 컴파일 실패면 조용히 아무것도 안 한다(`|| true`). 최악이 이 기능 전의 동작이다.
    - **게이트**: `RemoteTerminfoInstallerTests`가 진짜 `/bin/sh`+`tic`+`infocmp`로 (a) 없을 때 심고 (b) 있을 때 mtime 불변(스킵)을 검증한다 — 셸 시퀀싱이라 명령 문자열 테스트로는 안 보인다. E2E는 coin-dataset에서 지운 뒤 앱과 같은 base64로 재설치가 `FAIL→OK`가 됨을 확인했다.
    - **`running:` 경로(headless claude)에도 넣는다**: claude도 TUI라 원격 terminfo가 필요하다. 그래서 installer는 `run`이 있든(exec 직접) launcher가 붙든 양쪽 다 preamble로 실행된다.
    - 전제는 `RemoteCwdReporter`와 같다 — 키 인증(BatchMode)과 원격 `tic`. 비밀번호 호스트·`tic` 없는 호스트는 자동이 안 되고, 그게 degrade의 대상이다.
