...
<!-- 2026-08-31: 전수조사 중 확인. 2026-04-13 이후 손대지 않아 M12.5가 이미 없어진
     hook 시스템을 전제하고 있었다. 아래 표시는 그때 바로잡은 것. -->
...
세 항목 모두 **hook 시스템을 전제**로 쓰였는데, 그 시스템은 `9dfbc39`에서 프로세스 감지 +
화면 파싱으로 대체됐다. 배포할 hook이 없으므로 소켓 포워딩도, "hooks 미작동" 표시도 대상이 없다.

- [x] **PTY 출력 파싱** — `TerminalStateDetector`가 이걸 한다. 원격은 자식 프로세스를 볼 수 없어
      화면 파싱만으로 동작한다(CLAUDE.md "Known Issues" 참고).
- [~] ~~SSH 소켓 포워딩~~ / ~~원격 상태 미지원 표시~~ — hook 시스템과 함께 무의미해졌다.
      원격 상태를 더 정확히 알고 싶다면 새로 설계할 것.
