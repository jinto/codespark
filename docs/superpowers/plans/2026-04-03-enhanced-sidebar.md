...
**Goal:** cmux 수준의 사이드바 — workspace별 live session 목록, 상태 뱃지(Live/Idle), 자유 토글, 세션 이름 변경

**Architecture:** `workspace_summary_t`에 live session 배열을 추가하여 한 번의 API 호출로 사이드바 전체를 렌더링. Idle 상태는 Ghostty 출력 시간 기반으로 Swift에서 판단. 사이드바는 Finder 스타일 자유 토글로 각 workspace를 독립적으로 펼침/접힘.
...
### Task 1: Zig — WorkspaceSummary에 live session 배열 추가

**Files:**
- Modify: `libs/workspace-core/src/models.zig:169-182`
- Modify: `libs/workspace-core/src/store.zig:59-91`

- [ ] **Step 1: models.zig에 live_session_details 필드 추가**

`WorkspaceSummary` struct에 live session 배열 필드를 추가한다.
...
- [ ] **Step 2: store.zig listWorkspaceSummaries에 세션 로드 추가**

기존 aggregate 쿼리 이후, 각 workspace의 live session을 별도 쿼리로 로드한다. 기존 `liveSessionsForWorkspace` 패턴을 재사용한다.
...
pub fn listWorkspaceSummaries(self: *Store, allocator: std.mem.Allocator) StoreError![]models.WorkspaceSummary {
    ...
    while (try stmt.step()) {
        ...
        const live_details = try self.liveSessionsForWorkspace(allocator, ws_id);
...
참고: `liveSessionsForWorkspace`는 `workspaceDetail` 내부에서 이미 사용하는 패턴이다. store.zig에서 해당 함수를 확인하고 재사용한다.

- [ ] **Step 3: Zig 테스트 실행**

Run: `cd libs/workspace-core && zig build test`
Expected: PASS (기존 테스트가 새 필드에 적응)

- [ ] **Step 4: 커밋**

```bash
git add libs/workspace-core/src/models.zig libs/workspace-core/src/store.zig
git commit -m "feat(zig): add live_session_details to WorkspaceSummary"
```
...
**Files:**
- Modify: `libs/workspace-core/src/store.zig`
- Modify: `libs/workspace-core/tests/store_test.zig`
...
test "updateSessionTitle changes the session title" {
    ...
    const ws_id = try store.store.createWorkspace(std.testing.allocator, "test-ws");
    ...
    const detail = try store.store.workspaceDetail(std.testing.allocator, ws_id);
    defer {
        var d = detail;
...
```bash
git add libs/workspace-core/src/store.zig libs/workspace-core/tests/store_test.zig
...
**Files:**
- Modify: `libs/workspace-core/src/c_api.zig:70-77`
- Modify: `libs/workspace-core/include/workspace_core.h:76-83,156-159`

- [ ] **Step 1: c_api.zig workspace_summary_t에 live_session_details 추가**
...
pub const workspace_summary_t = extern struct {
    ...
    has_interrupted_sessions: bool,
    updated_at: i64,
    live_session_details: ?[*]workspace_session_summary_t,  // ← 새 필드
    live_session_detail_count: i32,                          // ← 새 필드
};
```

- [ ] **Step 2: listWorkspaceSummaries C API 함수에서 세션 배열 복사**

`workspace_service_list_workspace_summaries` 함수 내부에서 `live_session_details`를 C 배열로 변환한다. 기존 `workspace_service_workspace_detail`의 live session 변환 패턴을 참고한다.

- [ ] **Step 3: workspace_free_summaries에서 세션 배열 해제 추가**

`workspace_free_summaries` 함수에서 각 summary의 `live_session_details` 배열을 해제한다.

- [ ] **Step 4: update_session_title C API 함수 추가**

```zig
export fn workspace_service_update_session_title(
...
- [ ] **Step 5: workspace_core.h 헤더 업데이트**

`workspace_summary_t`에 새 필드 추가:
...
typedef struct workspace_summary_t {
    ...
    int64_t updated_at;
    workspace_session_summary_t *live_session_details;  /* 새 필드 */
...
Run: `cd libs/workspace-core && zig build`
Expected: 빌드 성공

- [ ] **Step 7: 커밋**

```bash
git add libs/workspace-core/src/c_api.zig libs/workspace-core/include/workspace_core.h
git commit -m "feat(zig): expose live_session_details in summary and add update_session_title C API"
```

---

### Task 4: Swift — WorkspaceSummaryViewData 확장 + protocol 추가

**Files:**
- Modify: `apps/macos/StatefulTerminal/Models/WorkspaceViewData.swift:3-9`
- Modify: `apps/macos/StatefulTerminal/Bridge/WorkspaceCoreClient.swift`
- Modify: `apps/macos/StatefulTerminal/Terminal/TerminalHostProtocol.swift`
- Modify: `apps/macos/StatefulTerminalTests/MockWorkspaceCoreClient.swift`

- [ ] **Step 1: WorkspaceViewData.swift에 liveSessionDetails 필드 추가**
...
- [ ] **Step 2: WorkspaceCoreClientProtocol에 updateSessionTitle 추가**
...
- [ ] **Step 3: LiveWorkspaceCoreClient.listWorkspaceSummaries()에서 세션 배열 파싱**
...
- [ ] **Step 4: LiveWorkspaceCoreClient.updateSessionTitle() 구현**
...
- [ ] **Step 6: MockWorkspaceCoreClient 업데이트**
...
기존 `WorkspaceSummaryViewData` 생성 코드에 `liveSessionDetails: []` 추가.
...
```bash
git add apps/macos/StatefulTerminal/Models/WorkspaceViewData.swift \
  ...
  apps/macos/StatefulTerminalTests/MockWorkspaceCoreClient.swift
...
```swift
init(core: WorkspaceCoreClientProtocol, terminalFactory: ...) {
    self.core = core
...
- [ ] **Step 1: SidebarView에 expandedWorkspaceIDs state 추가**
...
- [ ] **Step 2: WorkspaceSidebarRow를 토글 가능하게 변경**

workspace 이름 클릭 → 펼침/접힘 토글. 세션 행 클릭 → 해당 세션 활성화 + workspace 선택.

```swift
ForEach(model.workspaces) { workspace in
    VStack(alignment: .leading, spacing: 0) {
        // Workspace header — 클릭하면 토글
        WorkspaceSidebarRow(
            workspace: workspace,
...
- [ ] **Step 3: WorkspaceSidebarRow에 isExpanded 파라미터 추가**
...
- [ ] **Step 4: 선택된 workspace 자동 펼침**

`load()` 완료 후 첫 workspace를 자동으로 펼치도록 한다:
...
git commit -m "feat(swift): workspace toggle with live session list in sidebar"
```
...
확인 항목:
1. 사이드바에 workspace 이름 + disclosure triangle 표시
2. workspace 클릭 → 펼침/접힘 동작
3. 펼쳐진 workspace 아래에 live session 목록 표시
4. 세션 클릭 → 해당 세션으로 전환
5. 세션 더블클릭 → 이름 변경 가능
6. 10초 대기 → idle 뱃지 표시
7. 탭 바의 세션 이름도 변경 반영
