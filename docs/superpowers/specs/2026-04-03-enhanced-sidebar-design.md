...
Code Spark의 사이드바를 cmux 수준으로 강화한다. 현재 workspace별 aggregate count만 표시하는 사이드바를, 개별 세션 목록 + 상태 뱃지 + 이름 변경이 가능한 구조로 발전시킨다.

## Scope

| 포함 | 제외 (M2b) |
|------|-----------|
| 사이드바에 live session 목록 표시 | Workspace 생성/삭제/이름 변경 UI |
| 세션별 상태 뱃지 (Live/Idle/Closed) | Git 브랜치 + 경로 표시 |
| Workspace 자유 토글 (Finder 스타일) | 검색 (Cmd+K) |
| 세션 이름 변경 (더블클릭 inline edit) | Window state restoration |

## Design Decisions

- **사이드바 구조**: 자유 토글 (Finder 스타일) — 각 workspace를 독립적으로 펼치거나 접을 수 있음
- **데이터 흐름**: Summary에 세션 배열 포함 — `listWorkspaceSummaries()`가 세션 목록도 함께 반환
...
### WorkspaceSummaryViewData 확장

```swift
struct WorkspaceSummaryViewData: Identifiable {
    let id: String
    let name: String
...
### listWorkspaceSummaries SQL 변경

기존 workspaces 쿼리에 sessions LEFT JOIN 추가. workspace별 live session을 함께 로드.

## Sidebar UI Design

### SidebarView 변경

`SidebarView`에 `@State var expandedWorkspaceIDs: Set<String>` 추가.
...
### WorkspaceSidebarRow 변경

- workspace 이름 클릭 → 펼침/접힘 토글
...
| 파일 | 변경 |
|------|------|
| `libs/workspace-core/src/store.zig` | `listWorkspaceSummaries` JOIN 변경, `updateSessionTitle` 추가 |
| `libs/workspace-core/src/c_api.zig` | `workspace_summary_t` 확장, `update_session_title` 추가 |
| `libs/workspace-core/src/models.zig` | `SessionSummary` 타입 추가 |
| `libs/workspace-core/include/workspace_core.h` | C 헤더 업데이트 |
| `libs/workspace-core/tests/store_test.zig` | rename 테스트 추가 |
| `apps/macos/StatefulTerminal/Bridge/WorkspaceCoreClient.swift` | summary 파싱 확장, updateSessionTitle 추가 |
| `apps/macos/StatefulTerminal/Models/WorkspaceViewData.swift` | SessionSummary 타입, WorkspaceSummaryViewData 확장 |
| `apps/macos/StatefulTerminal/Models/AppModel.swift` | idleSessionIDs, renameSession, idle 타이머 |
| `apps/macos/StatefulTerminal/Views/SidebarView.swift` | expandedWorkspaceIDs, 토글, session 목록 렌더링 |
...
| `apps/macos/StatefulTerminalTests/MockWorkspaceCoreClient.swift` | updateSessionTitle mock |

## Verification

```bash
# Zig 테스트
cd libs/workspace-core && zig build test
